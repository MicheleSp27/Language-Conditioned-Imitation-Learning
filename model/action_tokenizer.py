...
class RT1ActionTokenizer():
    ...
    def tokenize(self, action: Dict[str, torch.Tensor]) -> torch.Tensor:
           ...
           # print("DoF Selected {}".format(k))
            ...
            if isinstance(a_space, spaces.Discrete):
               ...
               # print("Action {}".format(a))
                low = torch.tensor(a_space.low).to("cuda")
                high = torch.tensor(a_space.high).to("cuda")
                a = torch.clamp(a, low, high)
               # print("Action After Clamp {}".format(a))
                # Normalize the action.
                token = (a - low) / (high - low)
               # print("Action after normalization {}".format(token))
                # Bucket and discretize the action to vocab_size.
                token = token * (self._vocab_size - 1)
               # print("Action before tokenziation {}".format(token))
                token = token.to(torch.int32) # The size is (action_size) or (batch, action_size).
               # print("Action after tokenization {}".format(token))
        ...
        action_tokens = action_tokens.type(torch.int64)
